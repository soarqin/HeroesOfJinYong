...
void prepareData() {
    ...
    hojy::world::state::SkillData skills[2]{};
    skills[1].id = 1;
    skills[1].reqMp = 10;
    skills[1].damage[5] = 60;
    loadRecords(hojy::world::state::gSaveData.skillInfo, skills, 2);
}
...
void experienceRequirementsUseOriginalPotentialTier() {
    HOJY_CHECK_EQ(hojy::world::state::getExpForSkillLearn(1, 0, 0), 700);
    HOJY_CHECK_EQ(hojy::world::state::getExpForSkillLearn(1, 1, 0), 1400);
    HOJY_CHECK_EQ(hojy::world::state::getExpForSkillLearn(0, 0, 0), 1400);
    HOJY_CHECK_EQ(hojy::world::state::getExpForSkillLearn(1, 9, 0), 0);
    HOJY_CHECK_EQ(hojy::world::state::getExpForMakeItem(1, 0), 700);
    HOJY_CHECK_EQ(hojy::world::state::getExpForMakeItem(1, 90), 100);
}

void levelExperienceRejectsNonPositiveLevels() {
    HOJY_CHECK_EQ(hojy::world::state::getExpForLevelUp(0), 0);
...
void skillBookUsesItsOwnPropertyContract() {
    ...
    hojy::world::state::ItemData book{};
    book.addHp = 100;
    book.addPoisoned = -20;
    book.addStamina = 50;
    book.addMp = 100;
    ...
    hojy::world::state::applyBookChanges(&character, &book);
...
void postDamageChargesOnceAndKeepsStoredProgress() {
    ...
    HOJY_CHECK_EQ(hojy::world::state::calcSkillMpCost(hojy::world::state::gSaveData.skillInfo[1], 5), 30);
    hojy::world::state::postDamage(&character, 0, 5, 3, levelUp);
...
void injectedPostDamageRandomPreservesCallOrder() {
    auto character = makeCharacter();
    character.skillLevel[0] = 500;
    bool levelUp = false;
    ...
    hojy::world::state::postDamage(
        &character, 0, 5, 0, levelUp, random);

    HOJY_CHECK_EQ(character.skillLevel[0], 501);
    HOJY_CHECK_EQ(random.callCount(), 1U);
    HOJY_CHECK_EQ(random.calls()[0].minimum, 1);
...
void damageReportsOriginalExperience() {
    ...
    HOJY_CHECK_EQ(hojy::world::state::actDamage(
...
void roundEndAndLevelUpUseBatchContracts() {
    ...
    character.hurt = 40;
    character.poisoned = 20;
    HOJY_CHECK_EQ(hojy::world::state::actRoundEndDrain(&character, false), -4);
    HOJY_CHECK_EQ(character.hp, 6);

    character.hp = 4;
    HOJY_CHECK_EQ(hojy::world::state::actRoundEndDrain(&character, false), -3);
    ...
    hojy::world::state::actLevelup(&character, 2);
...
void injectedLevelupUsesTheProvidedRandomSourceForEveryGrowthRoll() {
    auto character = makeCharacter();
    character.level = 1;
    character.potential = 90;
    character.hpAddOnLevelUp = 1;
    character.medic = character.poison = character.depoison = 21;
    character.fist = character.sword = character.blade = 21;
    character.throwing = 21;
    hojy::battle::SequenceRandom random({0, 0, 0, 0, 0, 0, 0, 0, 0});

    hojy::world::state::actLevelup(&character, 1, random);

    HOJY_CHECK_EQ(random.callCount(), 9U);
}
...
int main() {
    try {
        ...
        injectedLevelupUsesTheProvidedRandomSourceForEveryGrowthRoll();
