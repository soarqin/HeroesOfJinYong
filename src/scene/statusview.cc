...
void StatusView::makeCache() {
    ...
    ttf->setColor(236, 200, 40); ttf->render(fmt::format(L"{:>3}", data_.attack), x3, y, true);
    y += lineheight;
    ttf->setColor(236, 236, 236); ttf->render(L"防禦力", x2, y, true);
    ttf->setColor(236, 200, 40); ttf->render(fmt::format(L"{:>3}", data_.defence), x3, y, true);
    y += lineheight;
    ttf->setColor(236, 236, 236); ttf->render(L"輕功", x2, y, true);
    ttf->setColor(236, 200, 40); ttf->render(fmt::format(L"{:>3}", data_.speed), x3, y, true);
    y += lineheight;
    ttf->setColor(236, 236, 236); ttf->render(L"醫療能力", x2, y, true);
    ttf->setColor(236, 200, 40); ttf->render(fmt::format(L"{:>3}", data_.medic), x3, y, true);
    y += lineheight;
    ...
    ttf->setColor(236, 200, 40); ttf->render(fmt::format(L"{:>3}", data_.poison), x3, y, true);
    y += lineheight;
    ttf->setColor(252, 148, 16); ttf->render(L"等級", x0, y, true);
    ttf->setColor(236, 200, 40); ttf->render(fmt::format(L"{:>3}", data_.level), x1, y, true);
    ttf->setColor(236, 236, 236); ttf->render(L"解毒能力", x2, y, true);
    ttf->setColor(236, 200, 40); ttf->render(fmt::format(L"{:>3}", data_.depoison), x3, y, true);
    y += lineheight;
    ttf->setColor(252, 148, 16); ttf->render(L"生命", x0, y, true);
    ttf->setColor(236, 200, 40); ttf->render(fmt::format(L"{:>3}/{:>3}", data_.hp, data_.maxHp), x1, y, true);
    ttf->setColor(236, 236, 236); ttf->render(L"拳掌功夫", x2, y, true);
    ttf->setColor(236, 200, 40); ttf->render(fmt::format(L"{:>3}", data_.fist), x3, y, true);
    y += lineheight;
    ttf->setColor(252, 148, 16); ttf->render(L"內力", x0, y, true);
    std::uint8_t r, g, b;
    std::tie(r, g, b) = mem::calcColorForMpType(data_.mpType);
    ttf->setColor(r, g, b); ttf->render(fmt::format(L"{:>3}/{:>3}", data_.mp, data_.maxMp), x1, y, true);
    ttf->setColor(236, 236, 236); ttf->render(L"御劍能力", x2, y, true);
    ttf->setColor(236, 200, 40); ttf->render(fmt::format(L"{:>3}", data_.sword), x3, y, true);
    y += lineheight;
    ttf->setColor(252, 148, 16); ttf->render(L"體力", x0, y, true);
    ttf->setColor(236, 200, 40); ttf->render(fmt::format(L"{:>3}/{:>3}", data_.stamina, data::StaminaMax), x1, y, true);
    ttf->setColor(236, 236, 236); ttf->render(L"耍刀技巧", x2, y, true);
    ttf->setColor(236, 200, 40); ttf->render(fmt::format(L"{:>3}", data_.blade), x3, y, true);
    y += lineheight;
    ttf->setColor(252, 148, 16); ttf->render(L"經驗", x0, y, true);
    ttf->setColor(236, 200, 40); ttf->render(fmt::format(L"{:>5}", data_.exp), x1, y, true);
    ttf->setColor(236, 236, 236); ttf->render(L"特殊兵器", x2, y, true);
    ttf->setColor(236, 200, 40); ttf->render(fmt::format(L"{:>3}", data_.special), x3, y, true);
    y += lineheight;
    ...
    ttf->setColor(236, 200, 40); ttf->render(fmt::format(L"{:>3}", data_.throwing), x3, y, true);
    if (showPotential) {
        y += lineheight;
        ttf->setColor(252, 148, 16); ttf->render(L"資質", x0, y, true);
        ttf->setColor(236, 200, 40); ttf->render(fmt::format(L"{:>5}", data_.potential), x1, y, true);
    }
    y = SubWindowBorder;
    ...
    for (int i = 0; i < data::LearnSkillCount; ++i) {
        ...
        ttf->setColor(236, 200, 40); ttf->render(util::big5Conv.toUnicode(mem::gSaveData.skillInfo[data_.skillId[i]]->name), x4, y, true);
    ...
    y += lineheight;
    if (data_.equip[0] >= 0) {
        ttf->render(util::big5Conv.toUnicode(mem::gSaveData.itemInfo[data_.equip[0]]->name), x0, y, true);
    }
    if (data_.learningItem >= 0) {
        ttf->render(util::big5Conv.toUnicode(mem::gSaveData.itemInfo[data_.learningItem]->name), x2, y, true);
    }
    y += lineheight;
    if (data_.equip[1] >= 0) {
        ttf->render(util::big5Conv.toUnicode(mem::gSaveData.itemInfo[data_.equip[1]]->name), x0, y, true);
    }
    if (data_.learningItem >= 0) {
        std::uint16_t expForItem = mem::getExpForSkillLearn(data_.learningItem, learningLevel, data_.potential);
        ttf->render(fmt::format(L"{:>5}/{:>5}", data_.expForItem, expForItem), x2, y, true);
    }
    cacheEnd();
}
