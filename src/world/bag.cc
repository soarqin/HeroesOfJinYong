...
void Bag::swap(Bag &other) noexcept {
    items_.swap(other.items_);
    orderedItems_.swap(other.orderedItems_);
...
void Bag::syncTo(BaseData &base) const {
    ...
    for (const auto &p: orderedItems_) {
        if (index >= ::hojy::content::BagItemCount) {
            break;
        }
        auto &item = base.items[index++];
        item.id = p.first;
        item.count = p.second;
    }
    for (; index < ::hojy::content::BagItemCount; ++index) {
        base.items[index] = {-1, 0};
    }
}
...
void Bag::syncToSave() {
    ...
    syncTo(*gSaveData.baseInfo.operator->());
    dirty_ = false;
}

void Bag::add(std::int16_t id, std::int16_t count) {
    if (count == 0 || id < 0 || id >= ::hojy::content::BagItemCount) { return; }
    auto &cnt = items_[id];
    cnt += count;
    if (cnt <= 0) {
        items_.erase(id);
...
bool Bag::remove(std::int16_t id, std::int16_t count) {
    if (id < 0 || id >= ::hojy::content::BagItemCount) { return false; }
